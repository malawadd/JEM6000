...
/** API */
import { fetchPriceETH, fetchPriceSeriesETH } from "@/services/api/stats"
...
onMounted(async () => {
	price.value = await fetchPriceETH()
	priceSeries.value = await fetchPriceSeriesETH({ timeframe: "1d" })
	...
	<Flex ref="widgetEl" direction="column" justify="between" :class="$style.wrapper">
		<Flex wide justify="between">
			<Flex direction="column" gap="8">
				<Text size="16" weight="500" color="primary">FraxEth Price</Text>
